...
public class MoneyBox {
    public MoneyBox(int[] defaultAmount) {
        money = defaultAmount;
    }

    private int money[];// 현재 보유한 화폐의 양, 박홍근:수정-> 초기에 화폐를 각각 5개씩 보유하도록 변경
    ...
    public boolean remainJudge() { 
        ...
        while(x !=0) { //거슬러줄 돈이 0 이 될때까지 로직 반복
            if (x>=10000 && simulationMoney[4] > 0) {
                simulationMoney[4]--;
                x -= 10000;
            }else if (x>=5000 && simulationMoney[3] > 0) {
                simulationMoney[3]--;
                x -= 5000;
            }else if (x>=1000 && simulationMoney[2] > 0) {
                simulationMoney[2]--;
                x -= 1000;
            }else if (x>=500 && simulationMoney[1] > 0) {
                simulationMoney[1]--;
                x -= 500;
            }else if (x>=100 && simulationMoney[0] > 0) {
                simulationMoney[0]--;
            	x -= 100;
            }else {
    ...
    public int[] remainingMoney() {//잔돈을 반환해주는 매서드 무결성 검사 이후이므로 데이터를 조작 
    	...
    	getMoneyBox = new int[5];//받은 돈을 돌려주었으므로 초기화
